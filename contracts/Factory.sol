...
contract Factory is IFactory {
    ...
    function createPool(
        IERC20 _asset,
        IERC20 _collateral
    ) external override returns (IPair _pool) {
        ...
        require(
            getPool[_asset][_collateral] == IPair(address(0)),
            'TimeswapFactory :: createPool : Pool Exists'
        );

        IPair _pair = IPair(new Pair{salt: keccak256(abi.encode(_asset,_collateral,fee,protocolFee))}(_asset,_collateral,fee,protocolFee));
