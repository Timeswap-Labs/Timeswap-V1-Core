...
describe('Borrow', () => {
  ...
  it('', async () => {
    ...
    for (let i = 0; i < tests.length; i++) {
      ...
      console.log('\n', `Checking for Borrow Test Case ${caseNumber + 1}`)
      ...
      try {
        ...
        try {
          ...
          if (borrowTxData.pair != undefined) {
            ...
            throw Error(borrowTxData.error)
          }
        } catch (error) {
          totalFailureCases++
        ...
        if (erm != 'minting error') {
          describe('', async () => {
            before(async () => {
              ...
              const mintTxData = await mintFixture(constructor, signers[0], mintParameters)
              pair = mintTxData.pair
              pairSim = mintTxData.pairSim
            })
            it(``, async () => {
