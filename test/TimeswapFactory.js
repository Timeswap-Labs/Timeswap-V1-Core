...
describe("createPool", () => {
  ...
  describe("success case", () => {
    it("Should create a pool", async () => {
      const result = await timeswapFactory.getPool(
        testToken1.address,
        testToken2.address,
        maturity
      );
...
describe("setFeeToSetter", () => {
  ...
  describe("success case", () => {
    it("Should change feeToSetter by feeToSetter", async () => {
      await timeswapFactory.setFeeToSetter(newFeeToSetter, {
        from: feeToSetter,
      });
  ...
  describe("fail case", () => {
    it("Should revert if not changed by feeToSetter", async () => {
      await expect(timeswapFactory.setFeeToSetter(newFeeToSetter)).to.be
        .reverted;
    });
  });
});
